...
def loadCheckpoint(model_file,model,args):
    assert os.path.exists(model_file),"the model file is not exist !"
    logger.info("Loading model and optimizer from checkpoint '{}'".format(args.resume_path))
    checkpoint = t.load(args.resume_path, map_location=args.device)
...
def saveCheckpoint(epoch,arch,model,extras,is_best,output_dir):
    ...
    if not isinstance(extras, dict):
        raise TypeError('extras must be either a dict or None')
    filename = arch.lower()+ '_checkpoint.pth.tar'
    filepath = os.path.join(output_dir, filename)
    filename_best =  arch.lower() + '_best.pth.tar'
    filepath_best = os.path.join(output_dir, filename_best)
