...
class Network(nn.Module):
  ...
  def _initialize_alphas(self):
    ...
    self.alphas_normal = nn.Parameter(torch.ones(k, num_ops),requires_grad=True)
    self.alphas_reduce = nn.Parameter(torch.ones(k, num_ops),requires_grad=True)
  ...
  def forward(self, input):
    ...
    for i, cell in enumerate(self.cells):
      if cell.reduction:
        ...
        bweights = binaryfunction.Quantizev1(weights,1)
      else:
