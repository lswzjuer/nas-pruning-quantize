...
def get_args():
    ...
    parser.add_argument('--step_size', type=float, default=3 / 255., help='step_size')
    parser.add_argument('--num_steps', type=int, default=5, help='num_steps')
    parser.add_argument('--attack', type=bool, default=True, help='attack')
...
def main():
    ...
    for sp in range(len(num_to_keep)):
        model = Network(args.init_channels + int(add_width[sp]), CIFAR_CLASSES, args.layers + int(add_layers[sp]), criterion,
                    ...
                    switches_normal=switches_normal, switches_reduce=switches_reduce, group=args.group,p=float(drop_rate[sp]))
        model = model.to(args.device)
        model_adv = AttackPGD(model,CONFIG)
        ...
        for epoch in range(epochs):
            ...
            if epoch < eps_no_arch:
                ...
                model.p = p
                model.update_p()
                t_top1, t_top5, t_loss = train(state_epochs+epoch,train_queue, valid_queue, model_adv, criterion, optimizer, optimizer_a,
                            args,monitors,logger, train_arch=False, adv=True,adv_weight=True)
            else:
                ...
                model.update_p()                
                t_top1, t_top5, t_loss = train(state_epochs+epoch,train_queue, valid_queue, model_adv, criterion, optimizer, optimizer_a, 
                    args,monitors,logger,train_arch=True,adv=True,adv_weight=True)
            
            v_top1, v_top5, v_loss = infer(state_epochs+epoch,valLoader, model_adv, criterion,args,monitors,logger,adv=True)
            
            if epoch >= eps_no_arch:
                # 将本epoch的解析结果保存
                arch_param = model.arch_parameters()
                normal_prob = F.softmax(arch_param[0]/0.2, dim=-1).data.cpu().numpy()
                reduce_prob = F.softmax(arch_param[1]/0.2, dim=-1).data.cpu().numpy()
                logger.info("normal: \n{}".format(normal_prob))
                logger.info("reduce: \n{}".format(reduce_prob))
...
def train(epoch, train_queue, valid_queue, model, criterion, optimizer, optimizer_a,
    ...
    # setting net_adv and net is training
    model.train()
    for step, (input, target) in enumerate(train_queue):
        ...
        if train_arch:
            ...
            if not adv:
                model.attack = False
                logits, _ = model(input_search,target_search)
                loss_a = criterion(logits, target_search)
            else:
                model.attack = False
                logits, _ = model(input_search,target_search)
                model.attack = True
                logits_a, _ = model(input_search,target_search)
                loss_a = criterion(logits_a, target_search) +  criterion(logits, target_search)

            # loss_a = criterion(logits, target_search)
            loss_a.backward()
            nn.utils.clip_grad_norm_(model.model.arch_parameters(), args.grad_clip)
            optimizer_a.step()


        optimizer.zero_grad()
        if adv and adv_weight:
            model.attack = False
            logits, _ = model(input,target)
            model.attack = True
            logits_a, _ = model(input,target)
            loss = criterion(logits, target) + criterion(logits_a, target)
        else:
            model.attack = False
            logits, _ = model(input,target)
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(model.model.weight_parameters(), args.grad_clip)
        optimizer.step()
...
def infer(epoch,valid_queue, model, criterion,args,monitors,logger,adv=False):
    objs = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        with torch.no_grad():
            if not adv:
                model.attack = False
                logits,inputs_adv = model(input,target)
                model.attack = True
            else:
                model.attack = True
                logits, inputs_adv = model(input, target)
...
if __name__ == '__main__':
    ...
    print('Total searching time: %ds', duration)
