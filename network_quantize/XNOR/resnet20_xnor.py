...
class BasicBlock_1w1a(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        self.conv1 = Layer.XNORConv2d_1w1a(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = Layer.XNORConv2d_1w1a(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                self.shortcut = nn.Sequential(
                     Layer.XNORConv2d_1w1a(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False),
                     nn.BatchNorm2d(self.expansion * planes)
                )
