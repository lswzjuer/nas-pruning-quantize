...
# @Last Modified time: 2020-09-29 13:30:13
...
__all__ = [  'vgg_small_1w1a_move','vgg_small_1w1a_dense','vgg_small_1w32a_dense',
             'vgg_small_1w1a','vgg_small_1w32a']
...
class VGG_SMALL_1W1A(nn.Module):
    ...
    def __init__(self, num_classes=10):
        ...
        self.conv4 = Layer.XNORConv2d_1w1a(256, 512, kernel_size=3, padding=1, bias=False)
        self.bn4 = nn.BatchNorm2d(512)
        self.conv5 = Layer.XNORConv2d_1w1a(512, 512, kernel_size=3, padding=1, bias=False)
...
class VGG_SMALL_1W1A_DENSE(nn.Module):
    ...
    def __init__(self, num_classes=10):
        ...
        self.fc =  Layer.XNORDense_1w1a(512*4*4, self.num_classes)
    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.fill_(1)
                m.bias.data.zero_()
            elif isinstance(m, nn.Linear) or inplace(m,Layer.XNORDense_1w1a):
                n = m.weight.size(1)
                m.weight.data.normal_(0, 0.01)
                if m.bias is not None:
                    m.bias.data.zero_()
...
class VGG_SMALL_1W32A_DENSE(nn.Module):
    ...
    def __init__(self, num_classes=10):
        ...
        self.fc =  Layer.XNORDense_1w32a(512*4*4, self.num_classes)
        self.bn6 = nn.BatchNorm1d(self.num_classes)
        self.scaleshift = ScaleAndShift(self.num_classes)
        self._initialize_weights()
    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.fill_(1)
                m.bias.data.zero_()
            elif isinstance(m, nn.Linear) or inplace(m,Layer.XNORDense_1w32a):
                n = m.weight.size(1)
                m.weight.data.normal_(0, 0.01)
                if m.bias is not None:
                    m.bias.data.zero_()
