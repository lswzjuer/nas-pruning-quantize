...
# @Last Modified time: 2020-10-10 19:58:31
...
def conv_1x1_bn_1w1a(inp,oup):
    return nn.Sequential(
        Layer.BiRealConv2d_1w1a(inp, oup, 1, 1, 0, bias=False),
        nn.BatchNorm2d(oup)
    )
...
class InvertedResidual_1w1a(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        if expand_ratio == 1:
            self.conv = nn.Sequential(
                # dw
                Layer.BiRealConv2d_1w1a(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False),
                nn.BatchNorm2d(hidden_dim),
                # pw-linear
            ...
            self.conv = nn.Sequential(
                # pw
                Layer.BiRealConv2d_1w1a(inp, hidden_dim, 1, 1, 0, bias=False),
                nn.BatchNorm2d(hidden_dim),
                # dw
                Layer.BiRealConv2d_1w1a(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False),
                nn.BatchNorm2d(hidden_dim),
                # pw-linear
                Layer.BiRealConv2d_1w1a(hidden_dim, oup, 1, 1, 0, bias=False),
                nn.BatchNorm2d(oup),
            )
