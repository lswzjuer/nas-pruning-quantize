...
# @Last Modified time: 2020-11-21 22:09:53
...
def conv_bn_1w1a(inp, oup, stride):
    return nn.Sequential(
        nn.Conv2d(inp, oup, 3, stride, 1, bias=False),
        nn.BatchNorm2d(oup)
    )
...
def conv_1x1_bn_1w1a(inp,oup):
    return nn.Sequential(
        Layer.BNNConv2d_1w1a(inp, oup, 1, 1, 0, bias=False),
        nn.BatchNorm2d(oup)
    )
...
class InvertedResidual_1w1a(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        if expand_ratio == 1:
            self.conv = nn.Sequential(
                # dw
                Layer.BNNConv2d_1w1a(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False),
                nn.BatchNorm2d(hidden_dim),
                # pw-linear
...
class MobileNet_1w1a(nn.Module):
    ...
    def __init__(self, num_classes=10, input_size=32, width_mult=1.):
        ...
        self.last_channel = make_divisible(last_channel * width_mult) if width_mult > 1.0 else last_channel
        # NOTE: change conv1 stride 2 -> 1 for CIFAR10
        self.features = [conv_bn_1w1a(3, input_channel, 1)]
        # building inverted residual blocks
        for t, c, n, s in interverted_residual_setting:
            output_channel = make_divisible(c * width_mult) if t > 1 else c
            for i in range(n):
